...
public class Page extends JavaHelpers
{
	...
	public Page(WebDriver webDriver)
	{
		...
		EventFiringWebDriver eventFiringWebDriver = new EventFiringWebDriver(getWebDriver());
		eventFiringWebDriver.register(seleniumWebDriverEventListener);
	}
	...
	{
		final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), timeOut);
	...
	protected boolean objectExists(By by, int seconds)
	{
		try
		{
			final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), seconds);
	...
	public boolean softAssert(Scenario scenario, List<String> actual, List<String> expected, String message)
	{
		if (!actual.equals(expected))
		{
			...
			scenario.write("Expected:[" + expected + "]");
			scenario.write("Actual:[" + actual + "]");
			return false;
		} else
		{
			scenario.write(message + " - Match:[" + expected + "]");
		}
		return true;
	}
	...
	{
		final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), TIMEOUT_ELEMENT);
	...
	{
		final Wait<WebDriver> webDriverWait = new WebDriverWait(getWebDriver(), TIMEOUT_ELEMENT);
	...
	{
		final Wait<WebDriver> webDriverWait = new WebDriverWait(getWebDriver(), timeout);
	...
	{
		// getWebDriver().manage().timeouts().pageLoadTimeout(getTimeOutPage(),
		// TimeUnit.SECONDS)
		sleep(3);
	}
	...
	{
		new WebDriverWait(getWebDriver(), TIMEOUT_ELEMENT)
	...
	{
		getWebDriver().manage().timeouts().implicitlyWait(value, TimeUnit.SECONDS);
	}
